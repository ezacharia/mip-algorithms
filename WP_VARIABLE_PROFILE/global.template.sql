...
-----
output_local_table  output_local_table
...
from ( select colname, SUM(N) as Ntotal
       from (select * from output_local_table  where val !='NA'));
...
from ( select colname, min(minval) as minval
       from (select * from  output_local_table  where minval !='NA' ));
...
from ( select colname, max(maxval) as maxval
       from (select * from  output_local_table   where maxval != 'NA'));
...
from ( select colname, val, SUM(N) as Ntotal
       from (select * from output_local_table  where val !='NA' and '%{categorical}'='True' )
       group by val);

insert into results
select "DatasetStatistics1" as type, colname as code, "NA" as categories, "NA" as header, Ntotal as gval
from ( select colname, val, SUM(N) as Ntotal
       from (select * from  output_local_table  where val =='NA' and '%{categorical}'='True' ) --NULL values
       group by val);
...
from (select * from  output_local_table  where val !='NA' and '%{categorical}'='True' and '%{valIsText}'='False');
...
from (select * from  output_local_table where val =='NA' and '%{categorical}'='True' and '%{valIsText}'='False'); --NULL values
